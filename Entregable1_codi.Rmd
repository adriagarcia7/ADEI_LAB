...
barplot(table(df$engineSize), main="Barplot engineSize Outcome - Factor", col="brown")
...
També creem variables factor auxiliars per a les variables numèriques

```{r}
###   Nova variable auxiliar(factor) age
df$age <-  2021 - df$year 
df$aux_age<-factor(df$age)
summary(df$age)
barplot(table(df$age),main="Barplot Age Outcome - Factor",col="brown")

###   variable auxiliar(factor) mileage
summary(df$model)
df$aux_model<-factor(df$model)
# It is a categorical(=factor) variable   NO PROBLEM

####   Variable auxiliar(factor) price
summary(df$price)
df$aux_price<-factor(df$price)

####   Variable auxiliar(factor) tax
summary(df$transmission)
df$aux_transmission<-factor(df$model)

####  Variable auxiliar(factor) mpg
summary(df$mileage)
df$aux_mpg<-factor(df$mpg)
...
### De les variables categòriques només comprovem que no tinguin cap NA
...
## DECISIONS: 
## 1. Eliminem les files on price és NA, ja que no ens interessa tenir missins a una variable resposta (fariem el mateix amb Audi però ja hem vist que no hi té cap NA)
...
```{r}
vars_cat<-names(df)[c(1,2,4,6,9,10,11,18)]
summary(df[,vars_cat])
...
```{r}
####    Discretization of all numerics, for example price
...
vars_quantitatives
names(df)
mout<-aq.plot(df[,c(3,5,7,8,12)],delta=qchisq(0.995,5),quan=0.995)

library(chemometrics)
summary(df[,vars_quantitatives])
mout<-Moutlier(df[,c(3,5,7,8,12)],quantile = 0.995, plot = TRUE)

ll<-which(mout$rd>5)
Boxplot(mout$rd)
df[ll,c(vars_res,vars_cat)]
df$mout <- 0
df$mout[ ll ]<-1
