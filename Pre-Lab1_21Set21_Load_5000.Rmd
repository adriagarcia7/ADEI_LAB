...
#### Comprovem NA de totes les variables
...
#Utilitzem per crear la variable binaria
...
#### Detectem outliers de totes les variables

```{r}
Boxplot(df$price)
var_out<-calcQ(df$price)
#nomes apareixen els de la linea seguent (per sobre)
abline(h=var_out$souts,col="red")
abline(h=var_out$souti,col="red")
abline(h=60000,col="blue",lwd=2)
...
#### Detectem errors(NA) de totes les variables

```{r}
# seleccionem preus iguals o per sota de 0€
...
#### Tractament errors i outliers
Posem valor NA a tots aquells errors i outliers prèviament detectats
```{r}
setNA<-which((df$price<=0) | (df$price>60000))
df[setNA,"price"]<-NA
```
...
## Create variables for the total number missing values, outliers and errors
```{r}
